...
## CI table-sweep test shipped 2026-05-08

Added `scripts/check-no-v1-table-refs.mjs` — greps `src/` and `supabase/functions/` for `.from('<table>')` and `'public.<table>'` references to the four dropped V1 tables (`loans`, `tenancies`, `costs`, `income`) and exits 1 with `file:line [table]` output if found. Allowlist: `src/lib/v1Frozen.ts` (intentional throw-guard string refs kept per #48 precedent) and `src/__tests__/check-no-v1-table-refs.test.ts` (the self-test fixture). Wired into `package.json` as `check:no-v1-refs` and inserted into the `verify` chain immediately after `check:no-any-disables`. Self-test at `src/__tests__/check-no-v1-table-refs.test.ts` (vitest) creates a sandbox tree with an offending file and asserts exit 1 + clean tree exits 0. Confirmed passing on current state — only the v1Frozen.ts allowlisted refs would match.
