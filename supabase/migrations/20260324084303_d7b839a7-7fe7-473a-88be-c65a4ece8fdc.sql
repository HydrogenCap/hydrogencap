...
BEGIN
  FOR v_compliance IN
    SELECT 
      ci.id, ci.property_id, ci.org_id, ci.compliance_type,
      ci.expiry_date, ci.responsible_party
  ...
  LOOP
    ...
    FROM properties WHERE id = v_compliance.property_id;

    v_days_until := v_compliance.expiry_date - CURRENT_DATE;

    IF v_days_until <= 14 THEN v_priority := 'urgent';
    ELSIF v_days_until <= 30 THEN v_priority := 'high';
    ELSIF v_days_until <= 60 THEN v_priority := 'normal';
    ELSE v_priority := 'low';
    END IF;

    INSERT INTO contractor_jobs (
      org_id, property_id, compliance_item_id, job_type, description,
      status, source, auto_created_at, priority
    )
    VALUES (
      v_compliance.org_id, v_compliance.property_id, v_compliance.id,
      ...
      'draft', 'auto_compliance', now(), v_priority
    )
