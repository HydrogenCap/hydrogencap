...
BEGIN
  ...
  IF v_access_id IS NULL THEN
    INSERT INTO public.shareholder_access (
      org_id, user_id, invite_id, access_level,
      can_view_financials, can_view_compliance, can_view_documents
    )
    VALUES (
      v_invite.org_id, v_user_id, v_invite.id, 'read_only',
      true, true, true
    )
...
BEGIN
  ...
  IF v_access_id IS NULL THEN
    INSERT INTO public.tenant_portal_access (
      org_id, tenant_id, tenancy_id, user_id, invite_id
    )
    VALUES (
      v_invite.org_id, v_invite.tenant_id, v_invite.tenancy_id,
      v_user_id, v_invite.id
    )
    RETURNING id INTO v_access_id;
  END IF;
