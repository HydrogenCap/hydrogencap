...
BEGIN
  ...
  LOOP
    ...
    -- Map V1 mortgage_type to V2 facility_type enum
    v_facility_type := CASE
      WHEN rec.mortgage_type ILIKE '%bridging%' THEN 'bridging'
      WHEN rec.mortgage_type ILIKE '%development%' THEN 'development_finance'
      WHEN rec.mortgage_type ILIKE '%mezzanine%' THEN 'mezzanine'
      WHEN rec.mortgage_type ILIKE '%second%charge%' THEN 'second_charge'
      WHEN rec.mortgage_type ILIKE '%overdraft%' THEN 'overdraft'
      WHEN rec.mortgage_type ILIKE '%director%' THEN 'directors_loan'
      ELSE 'term_mortgage'
    END;
    ...
    INSERT INTO public.loan_facilities (
      ...
      concat_ws(E'\n', rec.notes,
        ...
        CASE WHEN rec.mortgage_type IS NOT NULL THEN 'Original type: ' || rec.mortgage_type END,
