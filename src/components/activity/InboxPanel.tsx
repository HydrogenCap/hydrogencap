    ...
    <div className="space-y-6">
      ...
      {selectedIds.size > 0 && (
        <div className="sticky top-0 z-10 flex items-center gap-3 bg-background/95 backdrop-blur border rounded-lg p-3 shadow-sm">
          ...
          <Button size="sm" onClick={handleBulkAccept} disabled={isAcceptingAll} className="bg-emerald-600 hover:bg-emerald-700">
            <CheckCheck className="h-4 w-4 mr-1" />
            Accept All
          </Button>
          <Button size="sm" variant="destructive" onClick={() => setShowDeleteConfirm(true)} disabled={isBulkDeleting}>
      ...
      <Tabs defaultValue="pending" className="space-y-4">
        <TabsList>
          <TabsTrigger value="pending" className="gap-2">
            Ready for Review
            {readyDocs.length > 0 && (
              <Badge variant="secondary" className="h-5 min-w-5 px-1.5">{readyDocs.length}</Badge>
            )}
          </TabsTrigger>
