...
export async function buildFreeAgentAuthUrl(
  ...
  // CSRF protection: ask the backend to generate and persist a server-side
  // nonce. The returned `state` embeds that nonce; the callback validates it
  // against the oauth_states table and consumes it (single-use).
  const { data, error } = await supabase.functions.invoke<{ state: string }>(
    'freeagent-oauth-init',
    { body: { entityId, orgId, useSandbox } }
  );

  if (error || !data?.state) {
    throw new Error(error?.message || 'Failed to initialise FreeAgent OAuth');
  }
