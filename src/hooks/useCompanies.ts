...
export function useCompany(companyId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Get share classes
      const { data: shareClasses, error: scError } = await supabaseAny
        .from('share_classes')
        .select('*')
      ...
      const { data: shareholdings, error: shError } = await supabaseAny
        .from('shareholdings')
        .select(`
          *,
...
export function useCreateCompany() {
  ...
  return useMutation({
    mutationFn: async (input: {
      ...
      const { data: party, error: partyError } = await supabaseAny
        .from('parties')
        .insert({
          org_id: orgId,
          party_type: 'COMPANY',
      ...
      const { error: scError } = await supabaseAny
        .from('share_classes')
        .insert({
