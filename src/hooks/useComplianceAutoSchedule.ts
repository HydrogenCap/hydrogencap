...
export function useUpcomingRenewals(days: number = 90) {
  ...
  const renewals = useMemo<RenewalWindowItem[]>(() => {
    ...
    complianceItems.forEach(item => {
      ...
      // Calculate when the renewal window opens
      ...
      // Include items that are within the requested timeframe or already overdue
      if (daysUntilExpiry <= days || isOverdue) {
        items.push({
          complianceItemId: item.id,
          propertyId: item.property_id,
          propertyAddress: propertyMap.get(item.property_id) || 'Unknown Property',
    ...
    // Sort: overdue first, then by days until expiry
