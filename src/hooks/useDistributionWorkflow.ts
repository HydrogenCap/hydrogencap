...
export function useCreateDistributionRun() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const { data: run, error } = await supabaseAny
        .from('distribution_runs')
      ...
      if (allocations.length > 0) {
        ...
        const { error: lineError } = await supabaseAny
          .from('distribution_line_items')
          .insert(lineItems);
...
export function useApproveDistribution() {
  ...
  return useMutation({
    mutationFn: async (runId: string) => {
      const { data: auth } = await supabaseAny.auth.getUser();
...
export function useCalculateDistribution() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: investors } = await supabaseAny
        .from('investors')
