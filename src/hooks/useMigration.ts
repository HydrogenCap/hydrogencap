...
export function useRunMigrationStep() {
  ...
  return useMutation({
    mutationFn: async (functionName: string): Promise<MigrationResult> => {
      const orgId = await fetchUserOrgId();
      const { data, error } = await supabase.rpc(functionName as never, { p_org_id: orgId } as never);
...
export function useRunFullMigration() {
  ...
  return useMutation({
    mutationFn: async (): Promise<FullMigrationResult> => {
      const orgId = await fetchUserOrgId();
      const { data, error } = await supabase.rpc('run_v1_to_v2_migration' as never, { p_org_id: orgId } as never);
      if (error) throw error;
